        ...
        <div class="flex flex-wrap -m-4">
            @foreach($articles as $article)
                <div class="p-4 lg:w-1/3">
                    <div
                        class="card h-full px-8 pt-16 pb-24 overflow-hidden text-center relative">
                        <h6 class="tracking-widest text-xs title-font font-medium text-base dark:text-gray-500 mb-1">
                            {{ $article->category?->title }}
                        </h6>
                        <h2 class="title-font sm:text-2xl text-xl font-bold dark:text-white mb-3">{{ $article->title }}</h2>
                        <p class="leading-relaxed mb-3">
                            {{ Str::limit($article->description) }}
                        </p>
                        <a title="{{$article->title}}" href="{{ route('articles.show', ['article' => $article]) }}" class="text-indigo-600 dark:text-indigo-400 inline-flex items-center">Read more
